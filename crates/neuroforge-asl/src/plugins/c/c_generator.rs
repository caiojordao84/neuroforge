        ...
        match stmt {
            ...
            AslStatement::Declare(d) => {
                ...
                let t_str = match d.r#type {
                    ...
                    AslType::Int | AslType::Int32 | AslType::Sint8 | AslType::Int16 | AslType::Short => "int",

                    AslType::Uint | AslType::Uint32 | AslType::Uint8 | AslType::Uint16 | AslType::Byte => "unsigned int",
...
impl AslGenerator for CGenerator {
    ...
    fn generate(&mut self, program: &AslProgram) -> GeneratorOutput {
        ...
        let _has_setup = program.functions.iter().find(|f| f.name == "setup").is_some() || program.tasks.iter().find(|t| t.name == "setup").is_some();

        let _has_loop = program.functions.iter().find(|f| f.name == "loop").is_some() || program.tasks.iter().find(|t| t.name == "loop").is_some();

        let _has_main = program.functions.iter().find(|f| f.name == "main").is_some() || program.tasks.iter().find(|t| t.name == "main").is_some();
        ...
        for global in &program.globals {
            ...
            let t_str = match global.r#type {
                ...
                AslType::Uint | AslType::Uint32 | AslType::Uint16 | AslType::Byte => "unsigned int",
        ...
        for task in &program.tasks {
            ...
            if task.name == "setup" {
                ...
                for stmt in &task.body { self.gen_stmt(stmt, &mut lines, "  "); }
