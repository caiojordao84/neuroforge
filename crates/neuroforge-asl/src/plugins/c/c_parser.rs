...
use crate::types::asl_types::{
    ...
    AslProgram, AslStatement, AslExpr, AslMetadata, AslTask, AslFunction, AslType, AslPrint, AslLog,

    AslAssign, AslParam, AslExpressionStmt, AslIf, AslWhile, AslDeclare, AslBinary, AslCall,

    AslReturn, AslSwitch, AslSwitchCase, AslElseIf, AslDoWhile, AslDuration, AslLiteral, AslUnary, AslFor,

    AslDigitalOutput, AslAnalogOutput, AslDigitalInput, AslAnalogInput, AslDelay,

    AslPinMode, AslSerialBegin,

    BinaryOp, UnaryOp, PinModeKind
...
impl NeuroParser for CParser {
    ...
    fn parse(source: &str) -> Result<AslProgram, ParseError> {
        ...
        let tree = if parser.set_language(&tree_sitter_arduino::LANGUAGE.into()).is_ok() {
...
impl<'src> CVisitor<'src> {
    ...
    fn visit_function(&mut self, node: Node) -> AslFunction {
        ...
        let return_type_str = node.child_by_field_name("type")
    ...
    fn visit_params(&self, node: Node) -> Vec<AslParam> {
        ...
        for child in node.children(&mut cursor) {
            ...
            if child.kind() == "parameter_declaration" {
                ...
                let type_name  = child.child_by_field_name("type").map(|t| self.text(t).to_uppercase()).unwrap_or_else(|| "INT".into());

                let param_name = child.child_by_field_name("declarator").map(|d| self.text(d).to_string()).unwrap_or_else(|| "p".into());

                params.push(AslParam { name: param_name, r#type: type_name , ..Default::default() });
    ...
    fn visit_expr_stmt(&mut self, node: Node) -> AslStatement {
        ...
        match node.kind() {
            ...
            "assignment_expression" | "compound_assignment_expression" => {
                ...
                let op_text = self.text(node.child_by_field_name("operator").unwrap()).to_string();
                ...
                AslStatement::Assign(AslAssign { target, value: final_value })
    ...
    fn visit_expr(&mut self, node: Node) -> AslExpr {
        ...
        match node.kind() {
            ...
            "number_literal" => {
                ...
                if t.contains('.') { AslExpr::float(t.parse().unwrap_or(0.0)) }

                else { AslExpr::int(t.parse().unwrap_or(0)) }
            ...
            "call_expression" => {
                ...
                if let AslStatement::Expr(e) = stmt { e.expr }
            ...
            "parenthesized_expression" => {
                ...
                let inner = node.named_child(0).or_else(|| {
                    ...
                    if node.child_count() >= 3 { node.child(1) } else { None }
            ...
            kind @ ("binary_expression" | "logical_expression" | "relational_expression") => {
                ...
                println!("DEBUG visit_expr: kind='{}' text='{}'", kind, self.text(node));
                ...
                for child in node.children(&mut cursor) {
                    ...
                    } else if child.kind().contains("||") || child.kind().contains("&&") || 

                              child.kind() == "+" || child.kind() == "-" || child.kind() == "*" || child.kind() == "/" ||
                ...
                if operands.len() >= 2 {
                    ...
                    AslExpr::Binary(Box::new(AslBinary {
                        ...
                        op: match op_text {
                            ...
                            "and" | "AND" => BinaryOp::And, "or" | "OR" => BinaryOp::Or,
            ...
            kind => {
                ...
                if text.contains("||") || text.contains("&&") || text.contains("<=") || text.contains(">=") || text.contains("==") {
                     ...
                     let op_sym = if text.contains("||") { "||" } else if text.contains("&&") { "&&" } else if text.contains("<=") { "<=" } else if text.contains(">=") { ">=" } else { "==" };
                     ...
                     if parts.len() == 2 {
                         ...
                         return AslExpr::Binary(Box::new(AslBinary {
                             ...
                             op: match op_sym {
                                 ...
                                 "||" => BinaryOp::Or, "&&" => BinaryOp::And, "<=" => BinaryOp::Lte, ">=" => BinaryOp::Gte, _ => BinaryOp::Eq,
    ...
    fn visit_call(&mut self, node: Node) -> AslStatement {
        ...
        let args_node = node.child_by_field_name("arguments").expect("Call must have arguments node");
        ...
        match callee.as_str() {
            ...
            "delay" => {
                ...
                let ms = match args.get(0) {
                    ...
                    Some(AslExpr::Literal(l)) => l.value.as_u64().or_else(|| l.value.as_f64().map(|f| f as u64)).unwrap_or(0),

                    _ => 0, // Por agora simplificamos, idealmente AslDelay suportaria AslExpr

                };

                AslStatement::Delay(AslDelay { duration: AslDuration::from_ms(ms) })

            },

            "delayMicroseconds" => AslStatement::Delay(AslDelay {

                duration: AslDuration::from_us(args.get(0).and_then(|a| a.as_literal()).and_then(|l| l.value.as_i64()).unwrap_or(0) as u64),
            ...
            "Serial.print" | "Serial.println" => AslStatement::Print(AslPrint {
                ...
                args, newline: callee.contains("println"),
    ...
    fn visit_assignment_parts(&mut self, node: Node) -> (String, AslExpr) {
        ...
        let left  = self.text(node.child_by_field_name("left").unwrap()).to_string();
    ...
    fn visit_update(&self, node: Node) -> AslStatement {
        ...
        let var = text.trim_end_matches("++").trim_end_matches("--").to_string();

        let op = if text.ends_with("++") { BinaryOp::Add } else { BinaryOp::Sub };
        ...
        AslStatement::Assign(AslAssign {
            ...
            value: AslExpr::Binary(Box::new(AslBinary {
                ...
                op, left: AslExpr::var(&var), right: AslExpr::int(1),
    ...
    fn visit_declaration(&mut self, node: Node) -> Option<AslStatement> {
        ...
        if type_node.is_none() { return None; }
        ...
        if let Some(d) = declarator {
             ...
             let name = if d.kind() == "init_declarator" {
                 ...
                 self.text(d.child_by_field_name("declarator").unwrap()).to_string()
    ...
    fn visit_for(&mut self, node: Node) -> AslStatement {
        ...
        let init = node.child_by_field_name("initializer").map(|n| {
            ...
            if n.kind() == "declaration" { self.visit_declaration(n).into_iter().collect() }

            else { vec![self.visit_expr_stmt(n)] }

        }).unwrap_or_default();

        

        let cond = node.child_by_field_name("condition").map(|n| self.visit_expr(n)).unwrap_or_else(|| AslExpr::bool_val(true));

        let update = node.child_by_field_name("update").map(|n| self.visit_expr_stmt(n));
        ...
        if let Some(upd) = update { loop_body.push(upd); }

        

        let while_loop = AslStatement::While(Box::new(AslWhile { condition: cond, body: loop_body }));

        

        if init.is_empty() { while_loop }
    ...
    fn visit_while(&mut self, node: Node) -> AslStatement {
        ...
        let condition = node.child_by_field_name("condition").map(|c| self.visit_expr(c)).unwrap_or_else(|| AslExpr::bool_val(true));
    ...
    fn visit_switch(&mut self, node: Node) -> AslStatement {
        ...
        let discriminant = node.child_by_field_name("value").map(|v| self.visit_expr(v)).unwrap_or_else(|| AslExpr::int(0));
    ...
    fn visit_case(&mut self, node: Node) -> AslSwitchCase {
        ...
        let test = node.child_by_field_name("value").map(|v| self.visit_expr(v));
...
impl AslFunction {
    ...
    fn params_to_decl(&self) -> Vec<AslStatement> {
        ...
        self.params.iter().map(|p| AslStatement::Declare(AslDeclare {
...
"#;
        ...
        assert!(setup_task.body.iter().any(|s| matches!(s, AslStatement::SerialBegin(_))));
