//! ShimManager — registo e emissão de shims partilhado por todos os plugins.
...
/// Definição de um shim — bloco de código injectado no topo do ficheiro gerado.
#[derive(Debug, Clone)]
...
impl ShimManager {
    ...
    /// Marca um shim como obrigatório para a geração actual.
    /// Resolve dependências recursivamente. Ignora ciclos.
    pub fn require_shim(&mut self, name: &str) {
        if !self.registry.contains_key(name) {
            eprintln!(
                "[ShimManager] Aviso: shim '{}' pedido mas não registado para linguagem '{}'.",
                name,
        ...
        if self.required_shims.contains(name) {
            return; // Já requerido — evita ciclos infinitos
        }

        self.required_shims.insert(name.to_string());

        // Resolver dependências (clone para evitar borrow conflict)
    ...
    /// Retorna o bloco de código de todos os shims requeridos.
...
mod tests {
    ...
    fn make_manager() -> ShimManager {
        ...
        m.register_shim(
            ShimDefinition::new("lcd", "#include <LiquidCrystal_I2C.h>")
                .with_deps(vec!["wire"])
        );
        m
    }
    ...
    fn test_dependency_resolution() {
        ...
        assert!(m.has_requested("wire"), "dep wire deve ser resolvida automaticamente");
    }
    ...
    fn test_unknown_shim_does_not_panic() {
        let mut m = make_manager();
        m.require_shim("nonexistent"); // deve apenas emitir aviso, não panicar
    }
