...
use crate::types::asl_types::{
    ...
    AslProgram, AslFunction, AslStatement, AslExpr, AslType,
...
impl StGenerator {
    ...
    fn try_match_blink(&self, body: &[AslStatement]) -> Option<String> {
        ...
        let _dw1 = match &body[1] { AslStatement::DigitalOutput(d) => d, _ => return None };

        let d1 = match &body[2] { AslStatement::Delay(d) => d, _ => return None };

        let _dw2 = match &body[3] { AslStatement::DigitalOutput(d) => d, _ => return None };
    ...
    fn gen_stmt(&self, stmt: &AslStatement, level: usize) -> String {
        ...
        match stmt {
            ...
            AslStatement::Declare(d) => {
                ...
                let init = d.value.as_ref().map(|v| format!(" := {}", self.gen_expr(v))).unwrap_or_default();
            ...
            AslStatement::Delay(d) =>
                ...
                format!("{}(* delay({}ms) *)", ind, d.duration.total_ms()),
            ...
            AslStatement::DigitalInput(r) => {
                ...
                format!("{}pin_{} := digitalRead({});", ind, r.target, self.gen_expr(&r.pin))

            }

            AslStatement::AnalogInput(r) => {

                format!("{}pin_{} := analogRead({});", ind, r.target, self.gen_expr(&r.pin))
