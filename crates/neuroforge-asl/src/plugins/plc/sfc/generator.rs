...
//!
//! RT-8: geração de código alvo a partir de AslStateMachine.
//!
//! ## Targets suportados
//!
...
//!  - MicroPython:  and / or / not em minúsculas; True / False
...
impl SfcGenerator {
    ...
    /// Gera código para o target especificado.
    /// `target`: "st" | "cpp" | "rust" | "micropython"
    /// Devolve string vazia se não houver StateMachine no programa.
    ...
    fn gen_st(&self, sm: &AslStateMachine) -> String {
        ...
        // Variável de estado e inicialização
        out.push_str(&format!("VAR\n  {} : INT := {};\nEND_VAR\n\n",
            sm.state_var,
            sm.steps.iter().position(|s| s.name == sm.initial_step).unwrap_or(0)
        ));
    ...
    fn emit_expr_st(&self, expr: &AslExpr) -> String {
        match expr {
            ...
            AslExpr::Binary(b) => {
                ...
                format!("({} {} {})",
                    self.emit_expr_st(&b.left), sym, self.emit_expr_st(&b.right))
            }
            AslExpr::Call(c) => {
                let args = c.args.iter().map(|a| self.emit_expr_st(a)).collect::<Vec<_>>().join(", ");
    ...
    fn emit_expr_cpp(&self, expr: &AslExpr) -> String {
        match expr {
            ...
            AslExpr::Binary(b) => {
                ...
                format!("({} {} {})",
                    self.emit_expr_cpp(&b.left), sym, self.emit_expr_cpp(&b.right))
            }
            AslExpr::Call(c) => {
                let args = c.args.iter().map(|a| self.emit_expr_cpp(a)).collect::<Vec<_>>().join(", ");
                format!("{}({})", c.callee, args)
            }
            _ => "_unknown".to_string(),
        }
    ...
    fn emit_expr_rust(&self, expr: &AslExpr) -> String {
        match expr {
            ...
            AslExpr::Binary(b) => {
                ...
                format!("({} {} {})",
                    self.emit_expr_rust(&b.left), sym, self.emit_expr_rust(&b.right))
            }
            AslExpr::Call(c) => {
                let args = c.args.iter().map(|a| self.emit_expr_rust(a)).collect::<Vec<_>>().join(", ");
                format!("{}({})", c.callee, args)
            }
            _ => "_unknown".to_string(),
        }
    ...
    fn emit_expr_py(&self, expr: &AslExpr) -> String {
        match expr {
            ...
            AslExpr::Binary(b) => {
                ...
                format!("({} {} {})",
                    self.emit_expr_py(&b.left), sym, self.emit_expr_py(&b.right))
            }
            AslExpr::Call(c) => {
                let args = c.args.iter().map(|a| self.emit_expr_py(a)).collect::<Vec<_>>().join(", ");
                format!("{}({})", c.callee, args)
            }
            _ => "_unknown".to_string(),
        }
    ...
    fn emit_action_st(&self, stmt: &AslStatement) -> String {
        match stmt {
            AslStatement::Assign(a) =>
                format!("{} := {};", a.target, self.emit_expr_st(&a.value)),
            AslStatement::Expr(e) => {
    ...
    fn emit_action_cpp(&self, stmt: &AslStatement) -> String {
        match stmt {
            AslStatement::Assign(a) =>
                format!("{} = {};", a.target, self.emit_expr_cpp(&a.value)),
            AslStatement::Expr(e) => {
                if let AslExpr::Call(c) = &e.expr {
                    let args = c.args.iter().map(|a| self.emit_expr_cpp(a)).collect::<Vec<_>>().join(", ");
                    format!("{}({});", c.callee, args)
...
mod tests {
    ...
    // XML mínimo reutilizado do RT-7: Init →[StartBtn]→ Running →[TRUE]→ Init
...
"#
    ...
    fn generate_st_contains_case() {
        ...
        assert!(out.contains("CASE"),
            "ST deve conter CASE, output:\n{out}");
        assert!(out.contains("END_CASE"),
    ...
    fn generate_st_condition_reference() {
        let out = SfcGenerator::new().generate(&make_prog(), "st");
        assert!(out.contains("StartBtn"),
            "ST deve conter a condição StartBtn, output:\n{out}");
    }
    ...
    fn generate_cpp_contains_enum() {
        ...
        assert!(out.contains("enum"),
            "C++ deve conter enum, output:\n{out}");
        assert!(out.contains("STATE_Init"),
    ...
    fn generate_all_targets_nonempty() {
        ...
        for target in &["st", "cpp", "rust", "micropython"] {
            ...
            assert!(!out.is_empty(),
                "target '{}' não deve produzir output vazio", target);
        }
    }
}
