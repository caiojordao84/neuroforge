...
//! Uma rede LD (rung)    representada em PLCopen XML como grafos de n  s

//! ligados por `connectionPointIn/connection/@refLocalId`.

//!

//! Estrat  gia adoptada (conservadora, sem crate de grafo):
...
//!      b. Para cada coil, calcular a express  o booleana dos seus inputs:

//!         - recolher todos os contactos que chegam directamente    coil     OR

//!         - para cada caminho, seguir a cadeia de liga    es     AND

//!      c. Emitir `AslStatement::Assign` com o resultado.

//!   4. Emitir `AslProgram` com uma fun    o por POU.
...
//!   pou.body  : Vec<Box<Body>>               (Vec, n  o Option!)
...
//!       .ref_local_id: String  (n  o Option, n  o camelCase)
...
impl LdParser {
    ...
    pub fn parse(xml: &str) -> Result<AslProgram, LdParseError> {
        ...
        let project = parse_project(xml).map_err(|e| LdParseError::XmlError(e.to_string()))?;
        ...
        for pou in &pous.pou {
            ...
            // pou.body    Vec<Box<Body>>     usar o primeiro body dispon  vel

            let Some(body) = pou.body.first() else {
                continue;
            };

            // campo snake_case: body.ld  (n  o body.LD)

            let Some(ld) = body.ld.as_ref() else {
                continue;
            };
    ...
    fn lower_ld(ld: &plcopen::Body_LD_Inline) -> Vec<AslStatement> {
        ...
        for c in &ld.contact {
            ...
            // negated    serializado como string "true"/"false" pelo XSD
        ...
        for coil in &ld.coil {
            ...
            // Cada input directo da coil    uma rama OR

            let branches: Vec<AslExpr> = inputs
                .iter()
    ...
    //        Constru    o recursiva de express  o

    //

    // Percorre o grafo para tr  s a partir de um n  , construindo:
    ...
    /// Combina uma lista de express  es com OR.

    /// Lista vazia     FALSE; singleton     a pr  pria express  o.
    ...
    /// Connection.ref_local_id: String  (n  o Option, n  o camelCase)
...
mod tests {
    ...
    // Estrutura m  nima PLCopen TC6 XML v  lida.

    // Rede s  rie:    Rail(1)     A(2)     B(3)     Motor(coil 4)
...
"#
    ...
    fn serie_value_is_binary_and() {
        ...
        assert!(
            ...
            "rede s  rie deve produzir Binary(And), obtido: {:?}",
            assign.value
        );
    }
    ...
    fn contains_and(e: &AslExpr) -> bool {
        match e {
            AslExpr::Binary(b) => {
                matches!(b.op, BinaryOp::And) || contains_and(&b.left) || contains_and(&b.right)
            }
