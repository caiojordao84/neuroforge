...
//!
//! RT-10: inversão do LdParser. Converte cada AslStatement::Assign
//! numa rede LD (rung) com contacts e coil.
//!
//! ## Mapeamento AslExpr → topologia LD
//!
//!   Var(x)              → Rail → contact(x) → coil
//!   Binary(And, L, R)   → Rail → contact(L) → contact(R) → coil  (série)
...
//! ## localIds
//!
//!   Por função: contador global incremental.
//!   Por rung:   1 leftPowerRail + N contacts + 1 coil.

#![allow(dead_code)]

use crate::types::asl_types::{
    AslProgram, AslStatement, AslExpr, BinaryOp, UnaryOp,
};

pub struct LdGenerator;

/// Folha LD extraída da AslExpr.
#[derive(Debug, Clone)]
struct LdLeaf {
    var:     String,
    negated: bool,
...
/// Modo de ligação entre folhas.
#[derive(Debug, Clone, PartialEq)]
enum LdMode { Serie, Parallel }

impl LdGenerator {
    pub fn new() -> Self { Self }

    /// Gera PLCopen XML completo com body LD para cada função do programa.
    ...
    fn emit_rung(&self, target: &str, value: &AslExpr, id: &mut u32, out: &mut String) {
        ...
        *id += 1;
        ...
        // Decompor expressão em folhas + modo
        let (leaves, mode) = self.flatten_expr(value);

        // Emitir contacts e recolher os IDs das "pontas" que chegam à coil
    ...
    // ─── flatten_expr ───────────────────────────────────────────────────────────────
    //
    // Decompõe AslExpr em Vec<LdLeaf> + LdMode.
    //
    ...
    fn flatten_expr(&self, expr: &AslExpr) -> (Vec<LdLeaf>, LdMode) {
        match expr {
            AslExpr::Var(v) => {
                (vec![LdLeaf { var: v.name.clone(), negated: false }], LdMode::Serie)
            }
            AslExpr::Unary(u) if matches!(u.op, UnaryOp::Not) => {
                let name = self.expr_to_var_name(&u.expr);
                (vec![LdLeaf { var: name, negated: true }], LdMode::Serie)
            }
            ...
            _ => {
                // fallback: tratar como variável opaca
                let name = self.expr_to_var_name(expr);
                (vec![LdLeaf { var: name, negated: false }], LdMode::Serie)
            }
        }
    }

    /// Recolhe folhas de uma árvore AND (série).
    fn collect_and_leaves(&self, expr: &AslExpr, out: &mut Vec<LdLeaf>) {
        match expr {
            ...
            AslExpr::Unary(u) if matches!(u.op, UnaryOp::Not) => {
                let name = self.expr_to_var_name(&u.expr);
                out.push(LdLeaf { var: name, negated: true });
            }
            AslExpr::Var(v) => {
                out.push(LdLeaf { var: v.name.clone(), negated: false });
            }
            other => {
                out.push(LdLeaf { var: self.expr_to_var_name(other), negated: false });
            }
        }
    }

    /// Recolhe folhas de uma árvore OR (paralelo).
    ...
    /// Extrai nome de variável de uma AslExpr simples; fallback "_ld_expr".
...
mod tests {
    ...
    use crate::types::asl_types::{
        ...
        AslBinary, AslUnary, AslVarRef, BinaryOp, UnaryOp,
    };
    ...
    fn and(l: AslExpr, r: AslExpr) -> AslExpr {
        AslExpr::Binary(Box::new(AslBinary { op: BinaryOp::And, left: l, right: r }))
    }

    fn or(l: AslExpr, r: AslExpr) -> AslExpr {
        AslExpr::Binary(Box::new(AslBinary { op: BinaryOp::Or, left: l, right: r }))
    }
    ...
    fn generate_serie_has_contacts() {
        ...
        assert!(out.contains("<contact"),
            "série deve emitir contact, output:\n{out}");
        assert!(out.contains("<variable>A</variable>"),
            "deve conter variável A, output:\n{out}");
        assert!(out.contains("<variable>B</variable>"),
            "deve conter variável B, output:\n{out}");
    }

    #[test]
    fn generate_serie_has_coil() {
        let out = LdGenerator::new().generate(&prog_serie());
        assert!(out.contains("<coil"),
            "deve emitir coil, output:\n{out}");
        assert!(out.contains("<variable>Motor</variable>"),
            "coil deve ter variável Motor, output:\n{out}");
    }

    #[test]
    fn generate_serie_has_left_rail() {
        let out = LdGenerator::new().generate(&prog_serie());
        assert!(out.contains("<leftPowerRail"),
    ...
    fn generate_parallel_coil_has_two_connections() {
        ...
        // contacts também têm connection (1 cada) + coil tem 2
        assert!(count >= 4,
    ...
    fn generate_negated_contact_has_negated_true() {
        ...
        assert!(out.contains("<variable>Stop</variable>"),
            "deve conter variável Stop, output:\n{out}");
    }

    #[test]
    fn generate_roundtrip_pou_name() {
        let out = LdGenerator::new().generate(&prog_serie());
        assert!(out.contains("name=\"SerieRung\""),
