...
use crate::types::asl_types::{
    ...
    AslBinary, AslUnary, AslLiteral, AslFor, AslForRange, AslDuration,

    AslTask, AslFunctionBlock, AslFbField,
...
impl NeuroParser for StParser {
    ...
    fn parse(source: &str) -> std::result::Result<AslProgram, Self::Error> {
        ...
        if let Err(diags) = Self::validate_semantics(&program) {
            ...
            if diags.iter().any(|d| d.severity == DiagnosticSeverity::Error) {
                ...
                return Err(ParseError::Multiple(diags.iter().map(|d| d.to_string()).collect::<Vec<_>>().join("\n")));
    ...
    fn source_language() -> &'static str { "st" }
...
impl StVisitor {
    ...
    fn visit_declarations(&mut self, decls: Vec<PouDeclaration>) -> AslProgram {
        ...
        if !tasks.iter().any(|t| t.name == "setup") {
            ...
            tasks.push(AslTask {
                ...
                name: "setup".into(), body: vec![], ..Default::default() });
        ...
        if !tasks.iter().any(|t| t.name == "loop") {
            ...
            if let Some(first_task) = tasks.iter_mut().find(|t| t.name != "setup") {
                ...
                tasks.push(AslTask {
                    ...
                    name: "loop".into(), body: vec![], ..Default::default() });
    ...
    fn visit_program(&mut self, prog: iec61131::ProgramDecl) -> AslFunction {
        ...
        AslFunction { name: prog.name, return_type: None, params, body, doc: None , ..Default::default() }
    ...
    fn visit_fb(&mut self, fb: iec61131::FunctionBlockDecl) -> AslFunction {
        ...
        AslFunction { name: fb.name, return_type: None, params: vec![], body, doc: None , ..Default::default() }
    ...
    fn visit_function(&mut self, func: iec61131::FunctionDecl) -> AslFunction {
        ...
        AslFunction { name: func.name, return_type, params, body, doc: None , ..Default::default() }
    ...
    fn lower_expr(expr: Expression) -> AslExpr {
        ...
        match expr {
            ...
            Expression::Literal(l) => {
                ...
                AslExpr::Literal(AslLiteral { value: jv, ..Default::default() })
    ...
    fn visit_stmt(&mut self, stmt: Statement) -> Vec<AslStatement> {
        ...
        match stmt {
            ...
            Statement::For { control_var, start, end, step, body, .. } => {
                ...
                let to_exclusive = normalize::st_for_to_exclusive(&end_expr)
    ...
    fn first_for_range(body: &[AslStatement]) -> &crate::types::asl_types::AslForRange {
        ...
        body.iter().find_map(|s| {
            ...
            if let AslStatement::For(f) = s {
                ...
                if let crate::types::asl_types::AslFor::Range(r) = f.as_ref() { Some(r) } else { None }
...
"#;
    ...
    fn parse_st_program() {
        ...
        assert!(prog.tasks.iter().any(|t| t.name == "Main" || t.name == "loop"));
    ...
    fn assignment_value_is_not_debug_string() {
        ...
        let task   = prog.tasks.iter().find(|t| t.name != "setup").expect("task de loop");
    ...
    fn for_loop_emits_range_with_exclusive_to() {
        ...
        if let AslExpr::Literal(lit) = &r.to {
            ...
            assert_eq!(lit.value, serde_json::json!(10), "TO 9 deve ser normalizado para 10 exclusivo");
    ...
    fn debug_bin_op_repr() {
        ...
        let task2 = prog2.tasks.iter().find(|t| t.name != "setup").expect("task de loop");
