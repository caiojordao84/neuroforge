...
//!
//! A crate já expõe `Project`, `Body`, `LdObjects`, `FbdObjects`, `SfcObjects`,
//! `from_str` e `to_string`, cobrindo ST / IL / LD / FBD / SFC.
//!
//! Nomes gerados pelo schema XSD são verbosos mas estáveis:
//!   Root_project_InlineType_types_InlineType_pous_InlineType_pou_Inline (sem sufixo Type)
//!   pou.name e pou.pou_type são String diretos (não Option).
...
pub fn parse_project(xml: &str) -> Result<Project, PlcOpenXmlError> {
    plcopen::from_str::<Project>(xml)
        .map_err(|e| PlcOpenXmlError::Parse(e.to_string()))
}
...
pub fn serialize_project(project: &Project) -> Result<String, PlcOpenXmlError> {
    plcopen::to_string(project)
        .map_err(|e| PlcOpenXmlError::Serialize(e.to_string()))
}

/// Visitor simples para percorrer todos os POUs tipados do projeto.
///
/// `name` e `pou_type` são `String` diretos na struct gerada (não `Option`).
pub fn for_each_pou<F>(project: &Project, mut f: F)
where
    F: FnMut(&str, &str, &PlcPou),
{
    let Some(types) = project.types.as_ref() else { return; };
    let Some(pous) = types.pous.as_ref() else { return; };
...
/// Helper mínimo para criar um `AslPlcProgram` a partir de metadados do POU.
///
...
/// Tipos desconhecidos ficam em `Struct` para evitar perda semântica precoce.
pub fn map_iec_type(type_name: &str) -> AslType {
    match type_name.trim().to_ascii_uppercase().as_str() {
        ...
        | "BYTE" | "WORD" | "DWORD" | "LWORD" => AslType::Int,
        _ => AslType::Struct,
    }
}
