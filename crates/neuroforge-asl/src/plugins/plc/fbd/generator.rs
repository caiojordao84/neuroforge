...
//!
//! RT-9: inversão do FbdParser. Converte cada AslStatement numa rede FBD:
//!
...
//! localIds são contadores globais incrementais por função.
//! Posições calculadas automaticamente (espaçamento fixo).

#![allow(dead_code)]

use crate::types::asl_types::{
    AslProgram, AslStatement, AslExpr,
    BinaryOp, UnaryOp,
};
...
impl FbdGenerator {
    ...
    /// Gera PLCopen XML completo com body FBD para cada função do programa.
    ...
    fn emit_stmt(&self, stmt: &AslStatement, id: &mut u32, out: &mut String) {
        match stmt {
            ...
            AslStatement::TimerTon(t) => {
                ...
                self.emit_iec_block("TON", &t.instance, &[("IN", in_id), ("PT", pt_id)], &["Q"], id, out);
            }
            AslStatement::TimerTof(t) => {
                let in_id = self.emit_in_var(&self.emit_expr(&t.r#in), id, out);
                let pt_id = self.emit_in_var(&self.emit_expr(&t.pt), id, out);
                self.emit_iec_block("TOF", &t.instance, &[("IN", in_id), ("PT", pt_id)], &["Q"], id, out);
            }
            AslStatement::TimerTp(t) => {
                let in_id = self.emit_in_var(&self.emit_expr(&t.r#in), id, out);
                let pt_id = self.emit_in_var(&self.emit_expr(&t.pt), id, out);
                self.emit_iec_block("TP", &t.instance, &[("IN", in_id), ("PT", pt_id)], &["Q"], id, out);
            }
            AslStatement::CounterCtu(c) => {
                ...
                self.emit_iec_block("CTU", &c.instance,
                    &[("CU", cu_id), ("R", r_id), ("PV", pv_id)], &["Q", "CV"], id, out);
            }
            AslStatement::CounterCtd(c) => {
                ...
                let pv_id = self.emit_in_var(&self.emit_expr(&c.pv), id, out);
                self.emit_iec_block("CTD", &c.instance,
                    &[("CD", cd_id), ("LD", ld_id), ("PV", pv_id)], &["Q", "CV"], id, out);
            }
            AslStatement::LatchSr(l) => {
                ...
                self.emit_iec_block("SR", &l.instance,
                    &[("S1", s_id), ("R", r_id)], &["Q1"], id, out);
            }
            AslStatement::LatchRs(l) => {
                let r_id = self.emit_in_var(&self.emit_expr(&l.r), id, out);
                let s_id = self.emit_in_var(&self.emit_expr(&l.s), id, out);
                self.emit_iec_block("RS", &l.instance,
                    &[("R1", r_id), ("S", s_id)], &["Q1"], id, out);
            }
            AslStatement::TrigR(t) => {
                let clk_id = self.emit_in_var(&self.emit_expr(&t.r#in), id, out);
                self.emit_iec_block("R_TRIG", &t.instance,
                    &[("CLK", clk_id)], &["Q"], id, out);
            }
            AslStatement::TrigF(t) => {
                let clk_id = self.emit_in_var(&self.emit_expr(&t.r#in), id, out);
                self.emit_iec_block("F_TRIG", &t.instance,
                    &[("CLK", clk_id)], &["Q"], id, out);
            }
            AslStatement::Expr(e) => {
                if let AslExpr::Call(c) = &e.expr {
                    let arg_ids: Vec<(String, u32)> = c.args.iter().enumerate().map(|(i, arg)| {
                        let aid = self.emit_in_var(&self.emit_expr(arg), id, out);
                        (format!("IN{}", i + 1), aid)
                    }).collect();
                    let pins: Vec<(&str, u32)> = arg_ids.iter()
                        .map(|(f, id)| (f.as_str(), *id))
                        .collect();
                    self.emit_iec_block(&c.callee, &c.callee, &pins, &["OUT"], id, out);
                }
            }
            // outros statements não têm representação FBD directa
            _ => {}
        }
    }
    ...
    // ─── Emissores de nós FBD ──────────────────────────────────────────────────────

    /// Emite `<inVariable>` e devolve o localId atribuído.
    ...
    fn emit_out_var(&self, expression: &str, source_id: u32, id: &mut u32, out: &mut String) -> u32 {
        let cur = *id;
        let x = 250u32;
    ...
    /// Emite `<block>` com pins de entrada e saída; devolve o localId do bloco.
    fn emit_block(
        &self,
        type_name: &str,
        instance_name: &str,
    ...
    /// Atalho: emit_block + não emite outVariable (para FBs IEC sem assign explícito).
    fn emit_iec_block(
        &self,
        type_name: &str,
        instance_name: &str,
    ...
    fn emit_expr(&self, expr: &AslExpr) -> String {
        match expr {
            AslExpr::Literal(l) => {
                if l.value == serde_json::json!(true)  { return "TRUE".to_string(); }
                if l.value == serde_json::json!(false) { return "FALSE".to_string(); }
                l.value.to_string()
            }
            ...
            AslExpr::Binary(b) => {
                ...
                format!("({} {} {})",
                    self.emit_expr(&b.left), sym, self.emit_expr(&b.right))
            }
            AslExpr::Call(c) => {
...
mod tests {
                ...
                AslFunction {
                    ...
                    body: vec![
                        AslStatement::Assign(AslAssign {
                            ...
                            value:  AslExpr::Var(AslVarRef { name: "SensorA".to_string() }),
                        }),
                    ],
                },
            ],
                    ...
                    body: vec![
                        AslStatement::Assign(AslAssign {
                            ...
                            value: AslExpr::Call(Box::new(AslCall {
                                ...
                                args: vec![
                                    AslExpr::Var(AslVarRef { name: "A".to_string() }),
                                    AslExpr::Var(AslVarRef { name: "B".to_string() }),
                                ],
                            })),
                        }),
                    ],
        ...
        AslProgram {
            ...
            functions: vec![
                AslFunction {
                    ...
                    params: vec![], return_type: None,
                    body: vec![
                        AslStatement::TimerTon(AslTimerTon {
                            instance: "Timer1".to_string(),
                            r#in: AslExpr::Var(AslVarRef { name: "Start".to_string() }),
                            pt:   AslExpr::Var(AslVarRef { name: "Preset".to_string() }),
                        }),
                    ],
                },
            ],
    ...
    fn generate_contains_fbd_tag() {
        let out = FbdGenerator::new().generate(&prog_passthrough());
        assert!(out.contains("<FBD>"),
            "deve conter <FBD>, output:\n{out}");
        assert!(out.contains("</FBD>"),
            "deve conter </FBD>, output:\n{out}");
    }
    ...
    fn generate_passthrough_has_in_variable() {
        let out = FbdGenerator::new().generate(&prog_passthrough());
        assert!(out.contains("<inVariable"),
    ...
    fn generate_passthrough_has_out_variable() {
        let out = FbdGenerator::new().generate(&prog_passthrough());
        assert!(out.contains("<outVariable"),
    ...
    fn generate_passthrough_expression_sensora() {
        let out = FbdGenerator::new().generate(&prog_passthrough());
        assert!(out.contains("<expression>SensorA</expression>"),
    ...
    fn generate_ton_emits_block_ton() {
        ...
        assert!(out.contains("typeName=\"TON\""),
            "TimerTon deve emitir typeName=TON, output:\n{out}");
        assert!(out.contains("instanceName=\"Timer1\""),
    ...
    fn generate_roundtrip_pou_name() {
        let out = FbdGenerator::new().generate(&prog_passthrough());
        assert!(out.contains("name=\"PassthroughPOU\""),
