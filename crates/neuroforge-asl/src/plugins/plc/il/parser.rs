...
//!
//! Modelo de execução IL: acumulador + pilha para parênteses.
//! AND/OR são tratados como BinaryOp estruturado — nunca string crua.

use pest::Parser;
use pest_derive::Parser;

use crate::types::asl_types::{
    AslAssign, AslBinary, AslExpressionStmt, AslFunction, AslMetadata,
    AslParam, AslProgram, AslReturn, AslStatement, AslUnary, AslExpr,
    BinaryOp, UnaryOp,
};
...
#[derive(Debug, Clone)]
enum IlOp {
    Ld  { operand: String, negated: bool },
    St  { operand: String, negated: bool },
    /// AND obrigatoriamente tipado como BinaryOp::And — nunca string.
    And { operand: String, negated: bool },
    /// OR obrigatoriamente tipado como BinaryOp::Or — nunca string.
    Or  { operand: String, negated: bool },
    Xor { operand: String, negated: bool },
    Add(String), Sub(String), Mul(String), Div(String),
    Cmp { kind: CmpKind, operand: String },
    // _label e _cond prefixados: guardados para uso futuro (JMP resolution RT-9)
    Jmp { _label: String, _cond: JmpCond },
    Cal { name:  String, cond: JmpCond },
    Ret { cond: JmpCond },
    Set { operand: String },
    Rst { operand: String },
    Push,
...
enum CmpKind { Gt, Ge, Eq, Ne, Le, Lt }

#[derive(Debug, Clone, PartialEq)]
enum JmpCond { Unconditional, IfTrue, IfFalse }

#[derive(Debug, Clone)]
struct IlInst {
    // _label prefixado: guardado para futura resolução de JMP targets (RT-9)
...
impl IlParser {
    ...
    fn lower_pou(pair: pest::iterators::Pair<Rule>) -> AslFunction {
        ...
        for p in inner {
            match p.as_rule() {
                ...
                Rule::paren_open  => raw_insts.push(IlInst { _label: None, op: IlOp::Push }),
                Rule::paren_close => raw_insts.push(IlInst { _label: None, op: IlOp::Pop  }),
        ...
        AslFunction { name, return_type: None, params, body }
    }
    ...
    fn parse_var_block(pair: pest::iterators::Pair<Rule>) -> Vec<AslParam> {
        ...
        for p in pair.into_inner() {
            if p.as_rule() == Rule::var_decl {
                ...
                params.push(AslParam { name, r#type: typ });
            }
        }
        params
    }
    ...
    fn build_op(op: &str, o: String) -> Option<IlOp> {
        Some(match op {
            "LD"    => IlOp::Ld  { operand: o, negated: false },
            "LDN"   => IlOp::Ld  { operand: o, negated: true  },
            "ST"    => IlOp::St  { operand: o, negated: false },
            "STN"   => IlOp::St  { operand: o, negated: true  },
            "AND"   => IlOp::And { operand: o, negated: false },
            "ANDN"  => IlOp::And { operand: o, negated: true  },
            "OR"    => IlOp::Or  { operand: o, negated: false },
            "ORN"   => IlOp::Or  { operand: o, negated: true  },
            "XOR"   => IlOp::Xor { operand: o, negated: false },
            "XORN"  => IlOp::Xor { operand: o, negated: true  },
            "ADD"   => IlOp::Add(o),
            "SUB"   => IlOp::Sub(o),
            "MUL"   => IlOp::Mul(o),
            "DIV"   => IlOp::Div(o),
            "GT"    => IlOp::Cmp { kind: CmpKind::Gt, operand: o },
            "GE"    => IlOp::Cmp { kind: CmpKind::Ge, operand: o },
            "EQ"    => IlOp::Cmp { kind: CmpKind::Eq, operand: o },
            "NE"    => IlOp::Cmp { kind: CmpKind::Ne, operand: o },
            "LE"    => IlOp::Cmp { kind: CmpKind::Le, operand: o },
            "LT"    => IlOp::Cmp { kind: CmpKind::Lt, operand: o },
            "JMP"   => IlOp::Jmp { _label: o, _cond: JmpCond::Unconditional },
            "JMPC"  => IlOp::Jmp { _label: o, _cond: JmpCond::IfTrue  },
            "JMPCN" => IlOp::Jmp { _label: o, _cond: JmpCond::IfFalse },
            "CAL"   => IlOp::Cal { name:  o, cond: JmpCond::Unconditional },
            "CALC"  => IlOp::Cal { name:  o, cond: JmpCond::IfTrue  },
            "CALCN" => IlOp::Cal { name:  o, cond: JmpCond::IfFalse },
            "RET"   => IlOp::Ret { cond: JmpCond::Unconditional },
            "RETC"  => IlOp::Ret { cond: JmpCond::IfTrue  },
            "RETCN" => IlOp::Ret { cond: JmpCond::IfFalse },
        ...
        for inst in insts {
            match &inst.op {
                ...
                IlOp::Cmp { kind, operand } => {
                    let op = match kind {
                        CmpKind::Gt => BinaryOp::Gt,  CmpKind::Ge => BinaryOp::Gte,
                        CmpKind::Eq => BinaryOp::Eq,  CmpKind::Ne => BinaryOp::Neq,
                        CmpKind::Le => BinaryOp::Lte, CmpKind::Lt => BinaryOp::Lt,
                    };
                            ...
                            if let Some(cond_expr) = acc.clone() {
                                ...
                                stmts.push(AslStatement::If(Box::new(
                                    crate::types::asl_types::AslIf {
                                        ...
                                        then_branch: vec![AslStatement::Return(
                                            AslReturn { value: None }
                                        )],
                                        else_branch: None,
                                    }
                                )));
                ...
                IlOp::Cal { name, cond } => {
                    let call_expr = AslExpr::Call(Box::new(
                        crate::types::asl_types::AslCall { callee: name.clone(), args: vec![] }
                    ));
                ...
                // JMP: guardado com _ — sem representação ASL nesta fase
                IlOp::Jmp { .. } => {}
            }
        }
        stmts
    ...
    fn combine(acc: Option<AslExpr>, op: BinaryOp, rhs: AslExpr) -> AslExpr {
        match acc {
            None      => rhs,
            Some(lhs) => AslExpr::Binary(Box::new(AslBinary { op, left: lhs, right: rhs })),
        }
    }
    ...
    fn maybe_negate(expr: AslExpr, negated: bool) -> AslExpr {
        if negated { Self::negate(expr) } else { expr }
    }
}
...
"#;
    ...
    fn parse_and_produces_binary_and() {
        ...
        let func = &prog.functions[0];
        let AslStatement::Assign(assign) = &func.body[0] else { panic!("expected Assign") };
        ...
        assert!(matches!(bin.op, BinaryOp::And),
            "AND deve produzir BinaryOp::And, não string");
    }
    ...
    fn parse_or_produces_binary_or() {
        let prog = IlParser::parse(AND_OR_PROGRAM).unwrap();
        let func = &prog.functions[0];
        let AslStatement::Assign(assign) = &func.body[1] else { panic!("expected Assign") };
        assert_eq!(assign.target, "Out2");
        let AslExpr::Binary(bin) = &assign.value else { panic!("expected Binary") };
        assert!(matches!(bin.op, BinaryOp::Or),
            "OR deve produzir BinaryOp::Or, não string");
    }
    ...
    fn parse_andn_negates_operand() {
        ...
        let AslExpr::Binary(bin) = &assign.value else { panic!("expected Binary") };
        assert!(matches!(bin.op, BinaryOp::And));
        assert!(matches!(&bin.right, AslExpr::Unary(_)),
