...
use crate::types::typed_nodes::{BlockNode, BlockKind, ExprNode};
...
pub fn transform_block(block: &BlockNode, ctx: &TransformContext) -> Vec<AslStatement> {
    match &block.kind {
        ...
        BlockKind::For { init, condition, update, body } => {
            let init_stmt  = init.as_ref().map(|e| vec![AslStatement::Expr(AslExpressionStmt{expr: transform_expr(e, ctx)})]);
            let cond       = condition.as_ref().map(|e| transform_expr(e, ctx)).unwrap_or_else(|| AslExpr::bool_val(true));
            let update_stmt = update.as_ref().map(|e| vec![AslStatement::Expr(AslExpressionStmt{expr: transform_expr(e, ctx)})]).unwrap_or_default();
            let stmts      = transform_body(body, ctx);
        ...
        BlockKind::Switch { discriminant, cases, default } => {
            ...
            let mut asl_cases: Vec<AslSwitchCase> = cases.iter().map(|c| AslSwitchCase {
                test: Some(transform_expr(&c.value, ctx)),
                body: transform_body(&c.body, ctx),
            }).collect();
            if let Some(def_body) = default {
                asl_cases.push(AslSwitchCase { test: None, body: transform_body(def_body, ctx) });
            }
