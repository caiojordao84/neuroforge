//! Normalizador de AST — expande postfix side-effects e simplifica nós de hardware.
...
fn normalize_list(nodes: Vec<BaseNode>) -> Vec<BaseNode> {
    ...
    for node in nodes {
        ...
        // Side-effects extraídos do postfix vão antes do nó original
        ...
        // Unwrap ExpressionStatement que envolve um nó de hardware
        ...
        // Simplificação Pin(x) → x nos nós de hardware de GPIO
        ...
        if is_postfix && (op == "++" || op == "--") {
            if let Some(operand) = node.children.first() {
                if operand.node_type == "Identifier" {
                    ...
                    // Retorna referência ao temporário
...
mod tests {
    ...
    fn test_postfix_increment_expands() {
        ...
        // O nó retornado deve ser Identifier (__tmp_0)
    ...
    fn test_forloop_passthrough() {
        ...
        let mut stmts = vec![];
        let result = process_postfix_in_expr(node, &mut stmts);
        // ForLoop não deve ser processado — sem side-effects
