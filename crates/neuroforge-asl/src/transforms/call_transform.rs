...
use crate::types::typed_nodes::{CallNode, ExprNode};
    ...
    match call.name.as_str() {
        ...
        "pinMode" => {
            ...
            let is_output = match mode_arg {
                Some(arg) => if let crate::types::typed_nodes::ExprKind::StringLiteral(s) = &arg.kind { s == "OUTPUT" } else { false },
                None => true
            };
            let mode = if is_output { PinModeKind::Output } else { PinModeKind::Input };
            Some(AslStatement::PinMode(AslPinMode { pin, mode }))
        }
        "digitalWrite" => {
            let pin   = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let value = args.get(1).cloned().unwrap_or(AslExpr::int(0));
            Some(AslStatement::DigitalWrite(AslDigitalWrite { pin, value: DigitalValue::Expr(value) }))
        }
        "analogWrite" => {
            let pin   = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let value = args.get(1).cloned().unwrap_or(AslExpr::int(0));
        ...
        "digitalRead" | "analogRead" => {
            ...
            Some(AslStatement::Read(AslRead { pin, mode, target: result }))
        }

        // ── Serial ───────────────────────────────────────────────────────────
        "Serial.begin" | "serialBegin" => {
            let baud = args.get(0).cloned().unwrap_or(AslExpr::int(9600));
            Some(AslStatement::SerialBegin(AslSerialBegin { baud }))
        }
        "Serial.print" | "Serial.println" | "print" => {
            let value   = args.get(0).cloned().unwrap_or(AslExpr::str_val(""));
            let newline = call.name.ends_with("println");
            Some(AslStatement::Print(AslPrint { args: vec![value], newline }))
        }

        // ── Timing ───────────────────────────────────────────────────────────
        "delay" => {
            let ms = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            Some(AslStatement::Delay(AslDelay { milliseconds: ms }))
        }
        "delayMicroseconds" => {
            let us = args.get(0).cloned().unwrap_or(AslExpr::int(0));
        ...
        "servo.attach" | "servoAttach" => {
            ...
            Some(AslStatement::ServoAttach(AslServoAttach { var_name: name, pin, min_pulse: None, max_pulse: None, continuous: None }))
        }
        "servo.write" | "servoWrite" => {
            let angle = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let name  = call.object.clone().unwrap_or_else(|| "servo".into());
            Some(AslStatement::ServoWrite(AslServoWrite { var_name: name, angle, raw_microseconds: None }))
        }
        ...
        // ── I2C ──────────────────────────────────────────────────────────────
        "Wire.write" | "i2cWrite" => {
            let addr  = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let value = args.get(1).cloned().unwrap_or(AslExpr::int(0));
            Some(AslStatement::I2cWrite(AslI2cWrite { bus: AslExpr::int(0), address: addr, data: value }))
        }
        "Wire.read" | "i2cRead" => {
            let addr   = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let result = call.result_var.clone().unwrap_or_default();
            Some(AslStatement::I2cRead(AslI2cRead { bus: AslExpr::int(0), address: addr, target: result, length: AslExpr::int(1) }))
        }

        // ── SPI ──────────────────────────────────────────────────────────────
        "SPI.transfer" | "spiTransfer" => {
            let value  = args.get(0).cloned().unwrap_or(AslExpr::int(0));
            let result = call.result_var.clone().unwrap_or_default();
            Some(AslStatement::SpiTransfer(AslSpiTransfer { bus: AslExpr::int(0), cs_pin: AslExpr::int(0), tx_data: value, target: Some(result) }))
        }
        ...
        "pwmInit" => {
            ...
            Some(AslStatement::PwmInit(AslPwmInit { pin, freq, duty: AslExpr::int(0) }))
        }
        "pwmSetDuty" => {
            let pin  = args.get(0).cloned().unwrap_or(AslExpr::int(0));
        ...
        "rgbSet" => {
            ...
            let b   = args.get(3).cloned().unwrap_or(AslExpr::int(0));
            Some(AslStatement::RgbSet(AslRgbSet { pin_r: pin, pin_g: r, pin_b: g, r: AslExpr::int(0), g: AslExpr::int(0), b: AslExpr::int(0) }))
        }
