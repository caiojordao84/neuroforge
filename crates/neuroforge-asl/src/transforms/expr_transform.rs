...
use crate::types::asl_types::{AslExpr, AslBinary, BinaryOp, AslUnary, UnaryOp, AslCall, AslMember, AslConditional, AslIndex};
use crate::types::typed_nodes::{ExprNode, ExprKind};
...
pub fn transform_expr(node: &ExprNode, ctx: &TransformContext) -> AslExpr {
    match &node.kind {
        ...
        ExprKind::ArrayAccess { object, index } => {
            ...
            AslExpr::Index(Box::new(AslIndex { target: obj, index: idx }))
        }
        ...
        ExprKind::Call(call_node) => {
            let transformed_args: Vec<AslExpr> = call_node.args.iter()
        ...
        ExprKind::Ternary { condition, then_expr, else_expr } => AslExpr::Conditional(Box::new(AslConditional {
