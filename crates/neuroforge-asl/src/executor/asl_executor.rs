...
impl AslExecutor {
    ...
    pub fn run(source: &str, target: &TargetLanguage) -> Result<TranspileOutput, String> {
...

        {
            ...
            // Fallback (ex: recebemos c  digo fonte C++ para compilar para Rust)

            // Usa os novos pipes de transforma    o Universal
            ...
            match target {
                ...
                | TargetLanguage::Sfc => {
                    ...
                    return Err(format!(
                        ...
                        "O Parsing direto para {:?} n  o    suportado at   a Fase 2",
        ...
        match target {
            ...
            TargetLanguage::Fbd => {
                ...
                Err("FBD (Function Block Diagram) ainda n  o implementado     Fase 2 PLC".to_string())

            }

            TargetLanguage::Sfc => Err(

                "SFC (Sequential Function Chart) ainda n  o implementado     Fase 2 PLC".to_string(),
