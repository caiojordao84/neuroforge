...
use crate::types::asl_types::{
    ...
    AslProgram, AslFunction, AslTask, AslStatement, AslExpr, AslLiteral, AslFor,
    ...
    match stmt {
        ...
        AslStatement::For(mut s) => {
            ...
            match &mut *s {
                ...
                AslFor::Range(r) => { r.body = dce_body(std::mem::take(&mut r.body)); }

                AslFor::Each(e)  => { e.body = dce_body(std::mem::take(&mut e.body)); }

                AslFor::CStyle(c) => {

                    if bool_literal(&c.condition) == Some(false) { return DceResult::Remove; }
...
mod tests {
    ...
    fn prog_with(body: Vec<AslStatement>) -> AslProgram {
        ...
        p.functions.push(AslFunction {
            ...
            name: "test".to_string(), params: vec![], return_type: None, body, doc: None, ..Default::default() });
    ...
    fn replaces_if_true_with_then() {
        ...
        let stmt = AslStatement::If(Box::new(AslIf {
            ...
            else_body: Some(vec![AslStatement::Continue]), ..Default::default() }));
