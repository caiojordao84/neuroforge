...
use crate::types::asl_types::{
    ...
    AslProgram, AslFunction, AslTask, AslStatement, AslExpr, AslFor,
...
fn fold_stmt(stmt: &mut AslStatement) {
    ...
    match stmt {
        ...
        AslStatement::If(s) => {
            ...
            for st in &mut s.then_body { fold_stmt(st); }

            if let Some(eb) = &mut s.else_body { for st in eb { fold_stmt(st); } }
        ...
        AslStatement::For(s) => match &mut **s {
            ...
            AslFor::Range(r) => {
                ...
                for st in &mut r.body { fold_stmt(st); }
            ...
            AslFor::Each(e)  => {
                ...
                for st in &mut e.body { fold_stmt(st); }

            }

            AslFor::CStyle(c) => {

                for st in &mut c.init { fold_stmt(st); }

                fold_expr(&mut c.condition);

                for st in &mut c.update { fold_stmt(st); }

                for st in &mut c.body { fold_stmt(st); }
