//! Utilitários de arrays para os transforms ASL.
//! Migrado de: src/engine/asl/helpers/arrayUtils.ts

use serde_json::Value;

/// Representa um nó base do AST TypeScript (equivalente de BaseNode).
/// Usado apenas neste módulo para manter compatibilidade semântica com o TS.
#[derive(Debug, Clone)]
pub struct BaseNode {
...
/// Resolve o tamanho de uma expressão de dimensão de array.
...
/// Constrói um array JSON multidimensional inicializado a zero.
...
/// Cópia profunda de um valor JSON.
/// Equivalente a `deepCopyValue()` em TypeScript.
/// Em Rust, `Value::clone()` já é uma cópia profunda — esta função existe
/// apenas para manter a mesma API pública.
