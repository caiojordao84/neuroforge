...
impl ParseDiagnostic {
    ...
    pub fn error(msg: impl Into<String>) -> Self {
        ...
        Self { severity: DiagnosticSeverity::Error, message: msg.into(), span: None, code: None }

    }

    pub fn warning(msg: impl Into<String>) -> Self {

        Self { severity: DiagnosticSeverity::Warning, message: msg.into(), span: None, code: None }
...
impl fmt::Display for ParseDiagnostic {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(span) = &self.span {
            ...
            write!(f, "[{}] {} at {}: {}", self.severity, code, span, self.message)
...
pub trait NeuroParser: Sized {
    ...
    fn parse_to_json(source: &str) -> Result<String, Self::Error> {
        ...
        serde_json::to_string_pretty(&program)
            ...
            .map_err(|e| ParseError::Custom(e.to_string()).into())
...
pub trait NeuroParserExt: NeuroParser {
    ...
    fn validate_semantics(program: &AslProgram) -> Result<(), Vec<ParseDiagnostic>> {
        ...
        if let Err(e) = semver::Version::parse(&program.asl_version) {
            ...
            diags.push(ParseDiagnostic::error(format!("Invalid asl_version format: {}", e)).with_code("E002"));
...
pub mod normalize {
    ...
    pub fn bool_like(raw: &str) -> Option<AslExpr> {
        ...
        match raw {
            ...
            "HIGH" | "high" | "True" | "true" | "TRUE" | "1" | "set_high" | "SET_HIGH" | "SET" | "set" =>

                Some(AslExpr::Literal(AslLiteral { value: serde_json::json!(true), ..Default::default() })),

            "LOW" | "low" | "False" | "false" | "FALSE" | "0" | "set_low" | "SET_LOW" | "RESET" | "reset" =>

                Some(AslExpr::Literal(AslLiteral { value: serde_json::json!(false), ..Default::default() })),
    ...
    pub fn from_bool(v: bool) -> AslExpr {
        ...
        AslExpr::Literal(AslLiteral { value: serde_json::json!(v), ..Default::default() })
    ...
    pub fn assert_asl_op(op: &str) -> Result<(), ParseError> {
        ...
        if is_canonical_op(op) { Ok(()) }

        else { Err(ParseError::NormalizationError {
    ...
    pub fn st_for_to_exclusive(to_expr: &AslExpr) -> Option<AslExpr> {
        ...
        if let AslExpr::Literal(lit) = to_expr {
            ...
            if let Some(v) = lit.value.as_i64() {
                ...
                return Some(AslExpr::Literal(AslLiteral { value: serde_json::json!(v + 1) }));
...
mod tests {
    ...
    fn test_r4_bool_like() {
        ...
        assert_eq!(bool_like("HIGH").unwrap().as_literal().unwrap().value, serde_json::json!(true));

        assert_eq!(bool_like("low").unwrap().as_literal().unwrap().value, serde_json::json!(false));
    ...
    fn test_semver_validation() {
        ...
        impl NeuroParser for Dummy {
            ...
            fn parse(_: &str) -> Result<AslProgram, Self::Error> { Ok(AslProgram::default()) }

            fn source_language() -> &'static str { "dummy" }
