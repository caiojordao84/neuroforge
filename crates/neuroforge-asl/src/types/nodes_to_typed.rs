...
//! Necess  rio para a pipeline parse_to_asl_program em WASM bindings:
...
pub fn nodes_to_typed(prog: &crate::types::nodes::ProgramNode) -> ProgramNode {
    ...
    // Tenta usar os corpos j   separados no ProgramNode untyped.
    ...
    // Se os corpos estiverem vazios, tenta extrair das fun    es (fallback/compatibilidade).
    ...
    if setup_body.is_empty() && loop_body.is_empty() {
        ...
        // Se as fun    es setup/loop j   foram "extra  das" para os corpos,

        // filtramos elas da lista de fun    es regulares.
...
fn parse_raw_expr(s: &str) -> ExprNode {
    ...
    // Operadores bin  rios (simplificado)
        ...
        if let Some(idx) = s.find(op) {
...

            {
                ...
                if !left.is_empty() && !right.is_empty() {
                    ...
                    // Verificamos se estamos a meio de par  nteses (ex: "f(a == b)")
