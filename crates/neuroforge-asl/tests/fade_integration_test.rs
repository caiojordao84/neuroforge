...
"#;
    ...
    assert!(py_out.code.contains("ledPin = PWM(Pin(9), freq=1000)"));
    assert!(py_out.code.contains("while True:"));
    assert!(py_out.code.contains("ledPin.duty_u16(int((brightness) * 257))"));
    assert!(py_out.code.contains("if ((brightness <= 0) or (brightness >= 255)):"));
    ...
    assert!(main_rs.contains("const PIN_9: u8 = 9;"));
    assert!(main_rs.contains("let mut ledc = Ledc::new(peripherals.LEDC);"));
    assert!(main_rs.contains("let mut brightness = 0"));
    assert!(main_rs.contains("channel0.set_duty"));
