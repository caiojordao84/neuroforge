# Script de instalacao de dependencias do NeuroForge
# Execute: .\install-deps.ps1

Write-Host "=== NeuroForge - Instalacao de Dependencias ===" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path "src/engine") {
    ...
    Write-Host "[OK] src/engine removido" -ForegroundColor Green
} else {
    Write-Host "[OK] src/engine ja nao existe" -ForegroundColor Green
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "[ERRO] Falha ao instalar dependencias frontend" -ForegroundColor Red
    exit 1
}

Write-Host ""

# 3. Instalar dependencias do servidor QEMU
Write-Host "[3/4] Instalando dependencias do servidor QEMU..." -ForegroundColor Yellow
Set-Location server
npm install

if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Dependencias servidor instaladas" -ForegroundColor Green
} else {
    Write-Host "[ERRO] Falha ao instalar dependencias servidor" -ForegroundColor Red
    Set-Location ..
    exit 1
}
