<script lang="ts">
  ...
  async function testConnection() {
    ...
    try {
      ...
      // Use user-info endpoint for OpenAI, models for Anthropic
      ...
      const response = await fetch(testUrl, {
        method: 'GET',
        headers: isOpenAI ? { 'Authorization': `Bearer ${apiKey}` } : { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
      }).catch(() => null);
      ...
      if (response?.ok) {
        ...
        // For Anthropic, POST is required - if we get here, the key might still be valid
        // Try a minimal message to verify
        testStatus = 'success';
        testMessage = $providerState.type === 'openai' ? 'OpenAI key configured' : 'Anthropic key configured';
      }
    } catch (error) {
      testStatus = 'error';
