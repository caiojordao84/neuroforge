...
import { CppParser } from '../../src/engine/asl/plugins/cpp/CppParser';
import { RustParser } from '../../src/engine/asl/plugins/rust/RustParser';
import { PythonParser } from '../../src/engine/asl/plugins/python/PythonParser';
import { BlocklyParser } from '../../src/engine/blockly/BlocklyParser';
import { CodeToBlockly } from '../../src/engine/blockly/CodeToBlockly';
import { FlowToAst } from '../../src/engine/flow/FlowToAst';
import { PythonGenerator, PythonFlavor } from '../../src/engine/asl/plugins/python/PythonGenerator';
...
import { PatternDetector } from '../../src/engine/asl/plugins/analysis/PatternDetector';
import { Optimizer } from '../../src/engine/asl/plugins/optimizer/Optimizer';
import { SimulatorInterpreter } from '../../src/system/simulator/SimulatorInterpreter';
import { ProgramNode, PatternMatch, SourceMapEntry, AnalysisIssue } from '../../src/system/types';
...
export default function App() {
    ...
    const [boardId, setBoardId] = useState('esp32-devkitc-v4');
    const [boardConfig, setBoardConfig] = useState<any>(null);

    useEffect(() => {
        // Mock loading boards - in a real app this could be an API/Fetch
        const boards = {
            'esp32-devkitc-v4': {
                id: 'esp32-devkitc-v4',
                name: 'ESP32 DevKitC',
                gpio: [
                    { pin: 2, label: 'D2 (LED)', type: 'digital' },
                    { pin: 4, label: 'D4 (Servo)', type: 'digital' },
                    { pin: 5, label: 'D5 (Tone)', type: 'digital' },
                    { pin: 12, label: 'D12 (Neo)', type: 'digital' },
                    { pin: 13, label: 'D13', type: 'digital' },
                    { pin: 14, label: 'D14 (DHT)', type: 'digital' },
                    { pin: 15, label: 'D15', type: 'digital' },
                    { pin: 16, label: 'D16', type: 'digital' },
                    { pin: 17, label: 'D17', type: 'digital' },
                    { pin: 18, label: 'D18', type: 'digital' },
                    { pin: 19, label: 'D19', type: 'digital' },
                    { pin: 21, label: 'D21', type: 'digital' },
                    { pin: 22, label: 'D22', type: 'digital' },
                    { pin: 23, label: 'D23', type: 'digital' },
                    { pin: 25, label: 'D25', type: 'digital' },
                    { pin: 26, label: 'D26 (Analog)', type: 'digital' },
                    { pin: 27, label: 'D27', type: 'digital' },
                    { pin: 32, label: 'D32', type: 'digital' },
                    { pin: 33, label: 'D33', type: 'digital' },
                    { pin: 34, label: 'D34 (Joy X)', type: 'analog' },
                    { pin: 35, label: 'D35 (Joy Y)', type: 'analog' },
                    { pin: 36, label: 'D36', type: 'analog' },
                    { pin: 39, label: 'D39', type: 'analog' },
                    { pin: 0, label: 'D0', type: 'digital' },
                    { pin: 1, label: 'D1', type: 'digital' },
            ...
            'arduino-uno': {
                ...
                gpio: [
                    ...
                    { pin: 3, label: 'D3 (PWM)', type: 'digital' },
                    { pin: 4, label: 'D4', type: 'digital' },
                    { pin: 5, label: 'D5 (PWM)', type: 'digital' },
                    { pin: 6, label: 'D6 (PWM)', type: 'digital' },
                    { pin: 7, label: 'D7', type: 'digital' },
                    { pin: 8, label: 'D8', type: 'digital' },
                    { pin: 9, label: 'D9 (PWM)', type: 'digital' },
                    { pin: 10, label: 'D10 (PWM)', type: 'digital' },
                    { pin: 11, label: 'D11 (PWM)', type: 'digital' },
                    { pin: 12, label: 'D12', type: 'digital' },
                    { pin: 13, label: 'D13 (LED)', type: 'digital' },
            ...
            'raspberry-pi-pico': {
                ...
                name: 'RPi Pico',
                gpio: [
                    { pin: 25, label: 'LED (GP25)', type: 'digital' },
                    { pin: 0, label: 'GP0', type: 'digital' },
                    { pin: 1, label: 'GP1', type: 'digital' },
                    { pin: 26, label: 'GP26 (A0)', type: 'analog' },
                    { pin: 27, label: 'GP27 (A1)', type: 'analog' },
                    { pin: 28, label: 'GP28 (A2)', type: 'analog' }
                ]
            }
        };
        setBoardConfig((boards as any)[boardId]);
    }, [boardId]);

    const interpreterRef = useRef<SimulatorInterpreter | null>(null);
    const intervalRef = useRef<any>(null);
    const blocklyUserEdited = useRef(false); // tracks if user actively edited blocks
    ...
    // Handle Mode Switching: Sync Code to Blocks (visual only)
    useEffect(() => {
        if (mode === 'BLOCKS' && ast) {
            try {
                blocklyUserEdited.current = false; // Reset: we are loading from code
    ...
    const handleBlocklyChange = (xml: string) => {
        setBlocklyXml(xml);

        // Live Coding: Only update code if in BLOCKS mode and not currently syncing from code
        if (mode === 'BLOCKS') {
            try {
                const tempAst = new BlocklyParser().parse(xml);
                const flavor = pythonFlavor; // capture current flavor

                if (lang === 'PYTHON') {
                    const res = new PythonGenerator().generate(tempAst, flavor);
                    if (res.code !== pyCode) setPyCode(res.code);
                } else if (lang === 'RUST') {
                    const res = new RustGenerator().generate(tempAst);
                    if (res.code !== rustCode) setRustCode(res.code);
                } else {
                    const res = new CGenerator().generate(tempAst);
                    if (res.code !== cCode) setCCode(res.code);
                }
            } catch (e) {
                console.error("Blockly live-coding failed", e);
            }
        }
    };

    // Main Compile/Parse Loop — CODE is always source of truth
    // blocklyXml is NOT in the dependency array; Blockly is passive/visual only
    useEffect(() => {
        triggerParse();
    }, [cCode, rustCode, pyCode, flowNodes, flowEdges, mode, optimize, lang, transpileTarget, pythonFlavor]);
    ...
    const triggerParse = () => {
        try {
            ...
            // CODE is always the source of truth for AST.
            // Blockly is visual-only; it does NOT produce the AST.
            // Only FLOW mode produces its own AST (since it's an alternative input).
            if (mode === 'FLOW') {
            ...
            // Generate Interpreter Code (Only update state if NOT in Python mode to avoid loop)
            const simGen = new PythonGenerator().generate(newAst, 'MICROPYTHON');
            if (lang !== 'PYTHON') {
                setPyCode(simGen.code);
            }
            ...
            <header className="bg-slate-900 text-white p-3 flex items-center justify-between shadow-lg z-10">
                ...
                <div className="flex gap-2 items-center">
                    <select
                        ...
                        className="bg-slate-800 text-slate-300 text-xs px-2 py-1.5 rounded border border-slate-600 focus:outline-none focus:border-blue-500"
                    >
                        <option value="esp32-devkitc-v4">ESP32 DevKit</option>
                        <option value="arduino-uno">Arduino Uno</option>
                        <option value="raspberry-pi-pico">RPi Pico</option>
                    </select>
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="w-1/2 flex flex-col border-r border-slate-200 bg-white">
                    {mode === 'CODE' ? (
                        ...
                        <BlocklyEditor onXmlChange={handleBlocklyChange} xmlInput={incomingXml} boardConfig={boardConfig} />
                    ) : (
                        <FlowchartWrapper onGraphChange={(n: any[], e: any[]) => { setFlowNodes(n); setFlowEdges(e); }} boardConfig={boardConfig} />
                    )}
                </div>
