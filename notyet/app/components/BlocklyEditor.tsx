...
export const BlocklyEditor = ({ onXmlChange, xmlInput, boardConfig }: { onXmlChange: (xml: string) => void, xmlInput?: string, boardConfig: any }) => {
    const blocklyDiv = useRef<HTMLDivElement>(null);
    const workspace = useRef<any>(null);
    const boardRef = useRef<any>(boardConfig);

    useEffect(() => {
        boardRef.current = boardConfig;
    }, [boardConfig]);

    const getPinOptions = () => {
        if (!boardRef.current || !boardRef.current.gpio) {
            return [["None", "0"]];
        }
        return boardRef.current.gpio.map((g: any) => [g.label, g.pin.toString()]);
    };
    ...
    useEffect(() => {
        ...
        // Use a function for the dropdown to make it dynamic
        const dynamicPinDropdown = () => getPinOptions();

        // Define blocks (force re-definition to pick up dynamic dropdown)
    ...
    // Handle board change - refresh workspace to update dropdowns
    useEffect(() => {
        if (workspace.current && (window as any).Blockly) {
            const Blockly = (window as any).Blockly;
            workspace.current.getAllBlocks().forEach((block: any) => {
