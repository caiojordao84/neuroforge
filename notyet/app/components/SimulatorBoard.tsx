...
export const SimulatorBoard = ({ pins, logs, tones, lcd, dht, ultrasonic, ldr, ir, rgb, sevseg, neopixels, motors, mpu, oled, wifi, files, onPinChange, onDhtChange, onUltrasonicChange, onLdrChange, onIrPress, onKeyPress, onMpuChange }: any) => {
    ...
    const handleJoystick = useCallback((e: React.MouseEvent) => {
        ...
        // Map 0-100 to 0-4095
        onPinChange(34, Math.floor(x * 40.95));
        onPinChange(35, Math.floor((100-y) * 40.95)); // Y is usually inverted
    }, [onPinChange]);
        ...
        <div className="flex flex-col h-full bg-slate-200/50">
            <div className="flex-1 p-6 relative overflow-y-auto">
                 <div className="bg-emerald-800 rounded-lg p-6 shadow-2xl border-4 border-emerald-900 relative min-w-[500px]">
                    <div className="absolute top-2 left-4 text-xs text-emerald-100/50 font-mono">ESP32-SIM</div>
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                             {/* JOYSTICK */}
                             <div className="space-y-2">
                                <h4 className="text-emerald-100 text-xs font-bold uppercase tracking-wider"><LucideGamepad2 size={12} className="inline mr-1"/> Joystick (34,35)</h4>
                                <div 
                                    ref={joystickRef}
                                ...
                                >
                                    <div className="absolute w-8 h-8 bg-slate-400 rounded-full shadow-lg transform -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                                         style={{ left: `${(pins[34] / 4095) * 100}%`, top: `${(1 - pins[35] / 4095) * 100}%` }}></div>
                                </div>
                                <div className="text-center text-[10px] text-slate-400 font-mono">X: {pins[34]} Y: {pins[35]}</div>
                            </div>
