...
const LadderContactNode = ({ id, data }: any) => {
    ...
    return (
        <div className={`px-2 py-1 bg-white border-2 ${isNC ? 'border-red-400' : 'border-blue-400'} shadow-sm rounded flex flex-col items-center min-w-[80px] ${style}`}>
             ...
             <div className="text-[8px] text-slate-400">{data.pin ? `Pin ${data.pin}` : 'VAR'}</div>
...
export const FlowchartEditor = ({ onGraphChange }: { onGraphChange: (nodes: Node[], edges: Edge[]) => void }) => {
        ...
        <div className="w-full h-full flex flex-col relative">
            <div className="bg-slate-100 p-2 border-b flex flex-col gap-2">
                 ...
                 <div className="flex gap-2 overflow-x-auto items-center pb-2 border-b border-slate-200">
                    ...
                    <DragItem type="process" label="Process" color="bg-blue-500" meta={{code: 'digitalWrite(2, HIGH)'}} />
                ...
                <div className="flex gap-2 overflow-x-auto items-center pb-2 border-b border-slate-200">
                    ...
                    <DragItem type="state" label="State (LED)" color="bg-indigo-500" meta={{label: 'LED On', code: 'delay(100);', entry: 'digitalWrite(2,1);', exit: 'digitalWrite(2,0);'}} />
                ...
                <div className="flex gap-2 overflow-x-auto items-center">
                    ...
                    <DragItem type="ladder_contact" label="NO" color="bg-blue-400" meta={{label: 'IN_1', pin: 13, subType: 'NO'}} />
                    <DragItem type="ladder_contact" label="NC" color="bg-red-400" meta={{label: 'STOP', pin: 14, subType: 'NC'}} />
                    ...
                    <div className="w-64 bg-white border-l border-slate-200 shadow-lg p-4 flex flex-col gap-4 overflow-y-auto z-20">
                         ...
                         <div className="space-y-3 text-xs">
                             ...
                             {(selectedNode.type === 'ladder_contact' || selectedNode.type === 'ladder_coil') && (
                                 <div>
                                     <label className="block text-slate-500 font-bold mb-1">Pin (or Variable)</label>
                                     <input type="text" value={selectedNode.data.pin || ''} onChange={e => updateNodeData(selectedNode.id, { pin: e.target.value })} className="w-full border border-slate-300 rounded px-2 py-1 font-mono"/>
                                 </div>
                             )}
