...
export class FlowToAst {
    ...
    generate(nodes: Node[], edges: Edge[]): ProgramNode {
        // Reset
    ...
    private parseBlockContent(block: CfgBlock): BaseNode | null {
        ...
        if (type === 'ladder_timer') {
            ...
            if (subType === 'TON') {
                ...
                // TON ${id}
                if (${inVar}) {
                    if (!${id}_active) { ${id}_active = true; ${id}_start = millis(); }
                    if (millis() - ${id}_start >= ${pt}) ${id}_done = true;
                } else {
                    ${id}_active = false; ${id}_done = false;
                }`;
            } else if (subType === 'TOF') {
                code = `
                // TOF ${id}
                if (${inVar}) {
                    ${id}_done = true; ${id}_active = false;
                } else {
                    if (!${id}_active) { ${id}_active = true; ${id}_start = millis(); }
                    if (millis() - ${id}_start >= ${pt}) ${id}_done = false;
                }
                `;
            } else if (subType === 'TP') {
                code = `
                // TP ${id}
                if (${inVar} && !${id}_active) { ${id}_active = true; ${id}_start = millis(); ${id}_done = true; }
                if (${id}_active) {
                    if (millis() - ${id}_start >= ${pt}) { ${id}_done = false; if(!${inVar}) ${id}_active = false; }
                }
                `;
            }
        ...
        if (type === 'ladder_latch') {
            ...
            let code = '';
            if (subType === 'SR') { // Set Dominant
                code = `// SR ${id} \n if(${sVar}) ${id}_q = true; else if(${rVar}) ${id}_q = false;`;
            } else { // Reset Dominant
                code = `// RS ${id} \n if(${rVar}) ${id}_q = false; else if(${sVar}) ${id}_q = true;`;
            }
            return this.raw(code);
        }
        ...
        if (type === 'ladder_trig') {
            ...
            let code = '';
            if (subType === 'R_TRIG') {
                code = `// R_TRIG ${id} \n ${id}_q = (${clk} && !${id}_last); ${id}_last = ${clk};`;
            } else {
                code = `// F_TRIG ${id} \n ${id}_q = (!${clk} && ${id}_last); ${id}_last = ${clk};`;
            }
            return this.raw(code);
